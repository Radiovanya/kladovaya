...
function Registry({ page, data, search, setSearch, mode, setMode, drilldown, onClearDrilldown, onCustomer, onEdit, onArchive, onDelete, onTaskStatus, onContractDocument, onPayment, onCopyPhoto }: {
  ...
  if (page === "locations") {
    ...
    rows = data.customers.filter((x) => cell(x.fullName) || cell(x.phone) || cell(x.email)).map((x) => {
      ...
      const activeUnit = data.units.find((unit) => unit.id === activeContract?.unitId);
      const activeLocation = data.locations.find((location) => location.id === activeUnit?.locationId);
      ...
      return { id: x.id, customerId: x.id, cells: [<strong key="n">{x.fullName}</strong>, x.phone, x.email, activeUnit ? <span className="entity-info-cell" key="unit"><strong>№ {activeUnit.unitNumber}</strong><small>{activeLocation?.name ?? activeLocation?.address ?? "Локация не указана"}</small></span> : "—", <strong className={debt ? "danger-text" : ""} key="d">{money(debt)}</strong>] };
    });
    ...
    rows = data.charges.filter((x) => {
      ...
      const customer = data.customers.find((item) => item.id === contract?.customerId);
      return cell(contract?.contractNumber) || cell(unit?.unitNumber) || cell(customer?.fullName);
    }).map((x) => {
      const contract = data.contracts.find((c) => c.id === x.contractId)!;
      const unit = data.units.find((item) => item.id === contract.unitId);
      const location = data.locations.find((item) => item.id === unit?.locationId);
    ...
    rows = data.payments.filter((x) => { const contract = data.contracts.find((c) => c.id === x.contractId); const unit = data.units.find((item) => item.id === contract?.unitId); return cell(data.customers.find((c) => c.id === x.customerId)?.fullName) || cell(unit?.unitNumber); }).map((x) => {
      const contract = data.contracts.find((c) => c.id === x.contractId); const unit = data.units.find((item) => item.id === contract?.unitId); const location = data.locations.find((item) => item.id === unit?.locationId); const customer = data.customers.find((c) => c.id === x.customerId);
...
function CustomerDetails({ data, customerId, tab, setTab, onBack, onAdd, onQr, onContractDocument, onPayment, onViewScans, onUploadScan, onTelegram }: {
  ...
  const activeUnit = data.units.find((unit) => unit.id === active?.unitId);
  const activeLocation = data.locations.find((location) => location.id === activeUnit?.locationId);
  ...
  return (
    <section>
      ...
      <div className="summary-grid">
        <div><span>Кладовая</span><strong>{activeUnit ? `№ ${activeUnit.unitNumber}` : "Не назначена"}</strong><small>{activeLocation?.name ?? activeLocation?.address ?? "Локация не указана"}</small></div>
        <div><span>Арендная ставка</span><strong>{active ? money(active.monthlyRate) : "—"}</strong></div>
...
function TelegramInviteModal({ customerId, data, onClose }: {
  ...
  function shareInvite() {
    ...
    const text = `Здравствуйте! Подключите уведомления об оплате по договору ${selectedContract.contractNumber}. Нажмите ссылку и запустите бота:`;
      ...
      <section className="modal">
        ...
        {!contracts.length ? <div className="empty">У клиента нет активного договора</div> : <>
          <div className="form-grid">
            <label className="wide">Договор<select value={contractId} onChange={(event) => { setContractId(Number(event.target.value)); setUrl(""); setError(""); }}>{contracts.map((contract) => <option key={contract.id} value={contract.id}>{contract.contractNumber} · объект {data.units.find((unit) => unit.id === contract.unitId)?.unitNumber}</option>)}</select></label>
          </div>
        ...
        <div className="modal-actions">
          ...
          {url && <button className="button primary" onClick={shareInvite}><MessageCircle size={16} />Отправить в Telegram</button>}
...
function EntityModal({ modal, data, onClose, onSave }: { modal: Exclude<Modal, null>; data: AppData; onClose: () => void; onSave: (data: AppData, message: string) => void }) {
  ...
  const initialPaymentContract = data.contracts.find((item) => item.id === Number(editingPayment?.contractId)) ?? data.contracts.find((item) => item.status === "active") ?? data.contracts[0];
  const [error, setError] = useState("");
  const [customerId, setCustomerId] = useState(Number(editingPayment?.customerId ?? initialPaymentContract?.customerId ?? data.customers[0]?.id ?? 0));
  const [contractId, setContractId] = useState(Number(initialPaymentContract?.id ?? 0));
  ...
  function selectPaymentUnit(unitId: number) {
    setPaymentUnitId(unitId);
    const selectedUnit = data.units.find((item) => item.id === unitId);
    if (selectedUnit) setPaymentLocationId(selectedUnit.locationId);
    const unitContracts = data.contracts.filter((item) => item.unitId === unitId).sort((a, b) => b.startDate.localeCompare(a.startDate));
    const selectedContract = unitContracts.find((item) => item.status === "active") ?? unitContracts[0];
  ...
  function selectPaymentLocation(locationId: number) {
    ...
    const firstUnit = paymentUnits.find((unit) => unit.locationId === locationId);
    if (firstUnit) selectPaymentUnit(firstUnit.id);
  }
  function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError("");
    const form = new FormData(event.currentTarget);
    try {
      ...
      if (modal.type === "locations") upsert(next.locations, { id: modal.id ?? nextId(next.locations), name: input(form, "name"), address: input(form, "address"), description: input(form, "description"), isActive: editing?.isActive !== false });
        ...
        const candidate: Contract = { id: modal.id ?? nextId(next.contracts), customerId: Number(input(form, "customerId")), unitId: Number(input(form, "unitId")), contractNumber: input(form, "contractNumber"), startDate: input(form, "startDate"), endDate: input(form, "endDate"), monthlyRate: Number(input(form, "monthlyRate")), depositAmount: Number(input(form, "depositAmount")), billingDay: Number(input(form, "billingDay")), status: input(form, "status") as "active", terminationReason: value("terminationReason"), note: input(form, "note"), landlordType: input(form, "landlordType") as LandlordType, paymentIntervalMonths: Number(input(form, "paymentIntervalMonths")) as 1 | 3 | 6 | 12, firstPaymentDate: input(form, "firstPaymentDate"), advanceNoticeDays: Number(input(form, "advanceNoticeDays")) || 3 };
        validateActiveContract(candidate, next.contracts);
        next.charges = next.charges.filter((charge) =>
          charge.contractId !== candidate.id ||
          !charge.note.startsWith("Автоматический график") ||
          next.payments.some((payment) => payment.chargeId === charge.id)
        );
        upsert(next.contracts, candidate);
        next = syncContractPaymentSchedule(next, candidate.id);
    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
      <form className="modal" onSubmit={submit}>
        ...
        <div className="form-grid">
          ...
          {modal.type === "payments" && <>
            <label>Адрес<select value={paymentLocationId} onChange={(event) => selectPaymentLocation(Number(event.target.value))} required>{paymentLocations.map((location) => <option value={location.id} key={location.id}>{location.name || location.address}</option>)}</select></label>
            <label>Объект<select value={paymentUnitId} onChange={(event) => selectPaymentUnit(Number(event.target.value))} required>{paymentUnitsAtLocation.map((unit) => <option value={unit.id} key={unit.id}>№ {unit.unitNumber}</option>)}</select></label>
